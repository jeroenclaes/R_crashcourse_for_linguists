---
title: "Module 5: Elements of logistic regression"
...
# http://www.jeroenclaes.be/r_crashcourse_for_linguists/datasets/class10_Bresnan_et_al_2017.csv
...
# Load the course data from the course website to the object 'dataSet':
#http://www.jeroenclaes.be/r_crashcourse_for_linguists/datasets/class10_Bresnan_et_al_2017.csv
dataSet<-read_csv("http://www.jeroenclaes.be/r_crashcourse_for_linguists/datasets/class10_Bresnan_et_al_2017.csv")
# Load the dplyr package
library(dplyr)
# Convert all character values to Factor with `mutate_if`
...
library(readr)
library(dplyr)
dataSet <- read_csv("http://www.jeroenclaes.be/r_crashcourse_for_linguists/datasets/class10_Bresnan_et_al_2017.csv") %>% mutate_if(is.character, as.factor)
```
...
bootstrap <- function(model, replicates=1000) {
  suppressWarnings(suppressMessages( read_csv("http://www.jeroenclaes.be/r_crashcourse_for_linguists/datasets/class1O_Bresnan_et_al_2017_confints.csv")))
...
options(contr=c("contr.Sum", "contr.Poly"))
dataSet<-dataSet[-c(3703,3943 ),]
source("https://raw.githubusercontent.com/cran/MuMIn/master/R/r.squaredGLMM2.R")
```
